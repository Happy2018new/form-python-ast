...
如字面意思，`循环次数` 指的是代码块 `代码块` 需要被循环运行的次数。<br/>
...
在循环被终止后，`代码块` 中剩余的代码将不被执行，并且不会有新一轮循环开始。

```python
break
```

需要注意的是，您必须使用 `rof` 来关闭**循环语句**。<br/>
这跟**条件语句**中必须使用 `fi` 来关闭是一个道理。

另外一点是，`循环体` 中可以使用**条件语句**。<br/>
并且，**条件语句**语句中也可以出现**循环语句**。
...
## 示例八
在基于[示例七](#示例七-1)的基础上，如果我们将内存循环修改为循环 20 次，<br/>
